...
#define toBitInterleavingAndXOR(low, high, even, odd, temp, temp0, temp1)      \
  prepareToBitInterleaving(low, high, temp, temp0, temp1) even ^=              \
...
#define fromBitInterleaving(even, odd, low, high, temp, temp0, temp1)          \
  prepareFromBitInterleaving(even, odd, temp, temp0, temp1) low = temp0;       \
  high = temp1;

#define fromBitInterleavingAndXOR(even, odd, lowIn, highIn, lowOut, highOut,   \
                                  temp, temp0, temp1)                          \
  prepareFromBitInterleaving(even, odd, temp, temp0, temp1) lowOut =           \
      lowIn ^ temp0;                                                           \
  highOut = highIn ^ temp1;

void KeccakP1600_SetBytesInLaneToZero(KeccakP1600_plain32_state *state,
...
#endif
  toBitInterleavingAndAND(low, high, stateAsHalfLanes[lanePosition * 2 + 0],
                          stateAsHalfLanes[lanePosition * 2 + 1], temp, temp0,
                          temp1);
}
...
void KeccakP1600_AddByte(void *state, unsigned char byte,
                         unsigned int offset) {
  KeccakP1600_plain32_state *s = (KeccakP1600_plain32_state *)state;
  unsigned int lanePosition = offset / 8;
  unsigned int offsetInLane = offset % 8;
  uint32_t low, high;
  uint32_t temp, temp0, temp1;
  uint32_t *stateAsHalfLanes = s->A;
  ...
  toBitInterleavingAndXOR(low, high, stateAsHalfLanes[lanePosition * 2 + 0],
                          stateAsHalfLanes[lanePosition * 2 + 1], temp, temp0,
                          temp1);
}
...
void KeccakP1600_AddBytesInLane(KeccakP1600_plain32_state *state,
                                unsigned int lanePosition,
...
void KeccakP1600_AddLanes(KeccakP1600_plain32_state *state,
                          const unsigned char *data, unsigned int laneCount) {
#if (PLATFORM_BYTE_ORDER == IS_LITTLE_ENDIAN)
  ...
  for (lanePosition = 0; lanePosition < laneCount; lanePosition++) {
    ...
    uint32_t low = laneAsBytes[0] | ((uint32_t)(laneAsBytes[1]) << 8) |
                   ((uint32_t)(laneAsBytes[2]) << 16) |
                   ((uint32_t)(laneAsBytes[3]) << 24);
    uint32_t high = laneAsBytes[4] | ((uint32_t)(laneAsBytes[5]) << 8) |
                    ((uint32_t)(laneAsBytes[6]) << 16) |
                    ((uint32_t)(laneAsBytes[7]) << 24);
...
void KeccakP1600_AddBytes(void *state, const unsigned char *data,
  ...
  KeccakP1600_plain32_state *s = (KeccakP1600_plain32_state *)state;
  SnP_AddBytes(s, data, offset, length, KeccakP1600_AddLanes,
               KeccakP1600_AddBytesInLane, 8);
}

/* ---------------------------------------------------------------- */

void KeccakP1600_OverwriteBytesInLane(KeccakP1600_plain32_state *state,
                                      unsigned int lanePosition,
...
void KeccakP1600_OverwriteLanes(KeccakP1600_plain32_state *state,
                                const unsigned char *data,
  ...
  for (lanePosition = 0; lanePosition < laneCount; lanePosition++) {
    ...
    uint32_t even, odd, temp, temp0, temp1;
    uint32_t *stateAsHalfLanes = state->A;
    toBitInterleavingAndSet(low, high, stateAsHalfLanes[lanePosition * 2 + 0],
                            stateAsHalfLanes[lanePosition * 2 + 1], temp, temp0,
                            temp1);
  }
#endif
...
void KeccakP1600_OverwriteBytes(void *state, const unsigned char *data,
                                unsigned int offset, unsigned int length) {
  KeccakP1600_plain32_state *s = (KeccakP1600_plain32_state *)state;
  SnP_OverwriteBytes(s, data, offset, length, KeccakP1600_OverwriteLanes,
...
void KeccakP1600_OverwriteWithZeroes(void *state, unsigned int byteCount) {
  ...
  if (byteCount % 8 != 0)
    KeccakP1600_SetBytesInLaneToZero(s, byteCount / 8, 0, byteCount % 8);
}

/* ---------------------------------------------------------------- */

void KeccakP1600_ExtractBytesInLane(const KeccakP1600_plain32_state *state,
                                    unsigned int lanePosition,
  ...
  fromBitInterleaving(stateAsHalfLanes[lanePosition * 2],
                      stateAsHalfLanes[lanePosition * 2 + 1], low, high, temp,
                      temp0, temp1);
#if (PLATFORM_BYTE_ORDER == IS_LITTLE_ENDIAN)
...
void KeccakP1600_ExtractLanes(const KeccakP1600_plain32_state *state,
...
void KeccakP1600_ExtractBytes(const void *state, unsigned char *data,
                              unsigned int offset, unsigned int length) {
  const KeccakP1600_plain32_state *s =
      (const KeccakP1600_plain32_state *)state;
  SnP_ExtractBytes(s, data, offset, length, KeccakP1600_ExtractLanes,
...
void KeccakP1600_ExtractAndAddBytesInLane(
    const KeccakP1600_plain32_state *state, unsigned int lanePosition,
    const unsigned char *input, unsigned char *output, unsigned int offset,
    unsigned int length) {
  const uint32_t *stateAsHalfLanes = state->A;
  uint32_t low, high, temp, temp0, temp1;
  uint8_t laneAsBytes[8];
...
void KeccakP1600_ExtractAndAddLanes(const KeccakP1600_plain32_state *state,
  ...
  for (i = laneCount - 1; i >= 0; --i) {
    ...
    *(pO++) = *(pI++) ^ high;
#else
    fromBitInterleavingAndXOR(*(pS++), *(pS++), *(pI++), *(pI++), *(pO++),
                              *(pO++), t, x0, x1)
#endif
  }
#else
  ...
  for (lanePosition = 0; lanePosition < laneCount; lanePosition++) {
    ...
    laneAsBytes[6] = (high >> 16) & 0xFF;
    laneAsBytes[7] = (high >> 24) & 0xFF;
    ((uint32_t *)(output + lanePosition * 8))[0] =
        ((uint32_t *)(input + lanePosition * 8))[0] ^
        (*(const uint32_t *)(laneAsBytes + 0));
    ((uint32_t *)(output + lanePosition * 8))[1] =
        ((uint32_t *)(input + lanePosition * 8))[0] ^
        (*(const uint32_t *)(laneAsBytes + 4));
  }
#endif
}
/* ---------------------------------------------------------------- */
...
void KeccakP1600_ExtractAndAddBytes(const void *state,
  ...
  const KeccakP1600_plain32_state *s =
      (const KeccakP1600_plain32_state *)state;
  SnP_ExtractAndAddBytes(s, input, output, offset, length,
...
/* Chi step uses Wolf-style optimization: a^(~b&c) = a^(c&(b^c))
 * This avoids NOT which can be costlier on MCUs without ANDN. */
...
void KeccakP1600_Permute_Nrounds(void *state, unsigned int nRounds) {
  ...
  const uint32_t *pRoundConstants =
      KeccakF1600RoundConstants_int2 + (24 - nRounds) * 2;
  uint32_t *stateAsHalfLanes = s->A;
...
#undef SwapEO
  ...
  do {
    /* Code for 4 rounds, using factor 2 interleaving, 64-bit lanes mapped to
     * 32-bit words */
    switch (nRounds) {
    case 0:
...
void KeccakP1600_StaticInitialize(void) {
  /* Nothing to initialize at library load time. */
}
...
size_t KeccakF1600_FastLoop_Absorb(void *state, unsigned int laneCount,
                                   ...
                                   size_t dataByteLen) {
  size_t processed = 0;
  size_t rateInBytes = laneCount * 8;

  while (dataByteLen >= rateInBytes) {
    KeccakP1600_AddBytes(state, data, 0, (unsigned int)rateInBytes);
    KeccakP1600_Permute_24rounds(state);
    data += rateInBytes;
    dataByteLen -= rateInBytes;
    processed += rateInBytes;
  }
  return processed;
}
