...
// __________________ 
/* x4 wrappers: implement batched API by calling scalar SHAKE 4 times (unrolled) */

void shake128x4_init(shake128x4ctx *state)
{
  shake128_init(&state->s[0]);
  shake128_init(&state->s[1]);
  shake128_init(&state->s[2]);
  shake128_init(&state->s[3]);
}

void shake128x4_absorb_once(shake128x4ctx *state, const uint8_t *in0, const uint8_t *in1,
                            const uint8_t *in2, const uint8_t *in3, size_t inlen)
{
  shake128_absorb_once(&state->s[0], in0, inlen);
  shake128_absorb_once(&state->s[1], in1, inlen);
  shake128_absorb_once(&state->s[2], in2, inlen);
  shake128_absorb_once(&state->s[3], in3, inlen);
}

void shake128x4_squeezeblocks(uint8_t *out0, uint8_t *out1, uint8_t *out2, uint8_t *out3,
                              size_t nblocks, shake128x4ctx *state)
{
  shake128_squeezeblocks(out0, nblocks, &state->s[0]);
  shake128_squeezeblocks(out1, nblocks, &state->s[1]);
  shake128_squeezeblocks(out2, nblocks, &state->s[2]);
  shake128_squeezeblocks(out3, nblocks, &state->s[3]);
}
